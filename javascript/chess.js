...
class Pawn extends Piece {
    getAvailableSquares(board) {
        ...
        if (lastMove && lastMove.piece instanceof Pawn && Math.abs(lastMove.from[1] - lastMove.to[1]) === 2 && lastMove.piece.color !== this.color && Math.abs(lastMove.to[0] - r) === 1 && Math.abs(lastMove.to[1] - c) === 1) {
            moves.push([lastMove.to[0] + (this.color === "white" ? -1 : 1), lastMove.to[1]]);
        }
...
function isCheckmate(color) {
    return !piecesOnBoard.some(p => {
        ...
        const allMoves = getLegalMoves(p);
...
function getLegalMoves(piece) {
    const moves = piece.getAvailableSquares({
        isEmpty,
        isValidPosition,
        getPiece,
        enPassantTarget,
    ...
    return moves.filter(move => {
        ...
        const boardBackup = [...piecesOnBoard];

       
        piece.position = move;
        if (captured) {
            piecesOnBoard = piecesOnBoard.filter(p => p !== captured);
        }

        const inCheck = isCheck(piece.color);

        
        piece.position = originalPos;
        piecesOnBoard = boardBackup;
...
boardElement.addEventListener("click", (e) => {
    ...
    if (selectedPiece && sq.classList.contains("highlight")) {
        ...
        if (selectedPiece instanceof Pawn && Math.abs(r - selectedPiece.position[0]) === 2) {
            // Store the square behind the pawn as the en passant target
            const direction = selectedPiece.color === "white" ? -1 : 1;
            enPassantTarget = [r + direction, c];
        } else {
...
function getAllLegalMoves(color) {
    ...
    const available = getLegalMoves(piece);
...
function makeAIMove(difficulty = "easy") {
    ...
    if (moves.length === 0) {
        if(isCheck == true){
            alert(`${currentTurn} is in checkmate!`);
        }
        else{
            alert(`Stalemate!`)
        }
...
function getLegalMoves(piece) {
    const originalPos = [...piece.position];
    const moves = piece.getAvailableSquares({ isEmpty, isValidPosition, getPiece });
    const legalMoves = [];

    for (const move of moves) {
        const backupPieces = [...piecesOnBoard];
        const target = getPiece(move);
        if (target) piecesOnBoard = piecesOnBoard.filter(p => p !== target);
